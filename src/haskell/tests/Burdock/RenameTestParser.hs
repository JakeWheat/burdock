...
module Burdock.RenameTestParser
    (parseRenameTestFile
    ,RenameTest(..)
    ) where
...
parseRenameTestFile :: Text -> L.Text -> Either Text [RenameTest]
parseRenameTestFile sfn src = parseAnother $ prepSource src
  where
    parseAnother [] = Right []
    parseAnother (l:ls) | Just t <- readField "Test:" l = do
        (fs,ls') <- parseFiles ls
        (sc,ls'') <- parseScript ls'
        (res,ls''') <- parseResult ls''
        (RenameTest t fs sc res:) <$> parseAnother ls'''
    ...
    readBody [] = pure ("",[])
    parseScript :: [(Int,L.Text)] -> Either Text (L.Text, [(Int,L.Text)])
    parseScript (l:ls) | readBlankField "Script:" l
        = readBody ls
    parseScript ((n,_):_) = error $ formatError n $ "expected Script:"
    parseScript [] = error $ "expected script, got end of file"
    ...
    isRecognisedField (_,l) =
        ...
        ["Test:", "File:", "Script:", "Result:", "Errors"]
    readField nm (n,l) =
